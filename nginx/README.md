...
For every listen port an application uses, the generator also emits a catch-all `default_server` block that drops requests whose `Host` header or TLS SNI matches no application, so a direct hit on the server's bare IP never lands on whichever site Nginx loaded first. Each drop is logged as a block (`return 444`, `verdict=block`) so it shows in the dashboard's Blocked Requests. The HTTPS catch-all terminates TLS with a throwaway self-signed cert (`nginx/certs/freewaf-default.crt`, minted with `openssl` on first write, overridable via `NGINX_DEFAULT_TLS_CERT` / `NGINX_DEFAULT_TLS_KEY`) so the attempt can be logged; with no cert and no `openssl` it falls back to `ssl_reject_handshake on` (Nginx 1.19.4+), which still blocks but logs nothing. Turn the whole thing off under **Settings → Network → Bare IP**, or let one application own the slot by giving it the hostname `*` or enabling its `proxy.defaultServer`.
